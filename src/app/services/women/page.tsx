...
export default async function WomenServices() {
  const services = await prisma.service.findMany({ where: { gender: 'WOMEN', status: 'ACTIVE' } });

  return (
    <div className="container py-20 animate-fade-in">
      <div className="text-center max-w-3xl mx-auto mb-16">
        <h1 className="text-4xl md:text-5xl font-serif text-secondary mb-4">Women's Beauty</h1>
        <p className="text-muted text-lg">
          Luxurious hair treatments, relaxing facials, and complete beauty packages.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {services.map((service) => (
          <div key={service.id} className="card p-6 flex flex-col justify-between h-full border-l-4 border-pink-400">
            <div>
